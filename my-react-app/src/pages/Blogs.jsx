...
const Blogs = () => {
  ...
  useEffect(() => {
    fetch("https://tanzimportfolio.web.app/api/blogs")
      .then((res) => res.json())
      .then((data) => {
        setBlogs(data);
        setLoading(false);
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://tanzimportfolio.web.app/api/blogs", {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const handleDeleteBlog = async (id) => {
    ...
    try {
      const res = await fetch(`https://tanzimportfolio.web.app/api/blogs/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
                  ...
                  <article key={blog.id} className="blog-post featured">
                    ...
                    <div className="blog-post-media">
                      {blog.image_url && !blog.image_url.endsWith(".pdf") && (
                        <img
                          src={`https://tanzimportfolio.web.app${blog.image_url}`}
                          alt="Blog Visual"
                      ...
                      {blog.video_url && (
                        <video controls className="blog-video">
                          <source
                            src={`https://tanzimportfolio.web.app${blog.video_url}`}
                            type="video/mp4"
                          />

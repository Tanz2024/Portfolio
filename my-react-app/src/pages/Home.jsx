...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Blogs
        const blogsRes = await fetch("https://tanzimportfolio.web.app/api/blogs");
        if (blogsRes.ok) {
        ...
        // Fetch Projects
        const projectsRes = await fetch("https://tanzimportfolio.web.app/api/projects");
        if (projectsRes.ok) {
        ...
        // Fetch Testimonials
        const testimonialsRes = await fetch("https://tanzimportfolio.web.app/api/testimonials");
        if (testimonialsRes.ok) {
          setTestimonials(await testimonialsRes.json());
        }

        // Authenticate User
        const authRes = await fetch("https://tanzimportfolio.web.app/authenticate", { credentials: "include" });
        if (authRes.ok) {
        ...
        // Fetch Profile Image
        const profileRes = await fetch("https://tanzimportfolio.web.app/api/public/profile-image");
        if (profileRes.ok) {
          const data = await profileRes.json();
          if (data.profileImageURL) {
            setProfileImageURL("https://tanzimportfolio.web.app" + data.profileImageURL);
          }
        }
        
        // Fetch Bio Data
        const bioRes = await fetch("https://tanzimportfolio.web.app/api/public/profile");
        if (bioRes.ok) {
  ...
  const handleResumeDownload = () => {
    window.open("https://tanzimportfolio.web.app/api/resume/download", "_blank");
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("https://tanzimportfolio.web.app/api/resume", {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const handleProfileImageUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("https://tanzimportfolio.web.app/api/profile-image", {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const handleBioSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://tanzimportfolio.web.app/api/user/profile", {
        method: "PUT",
        credentials: "include",
  ...
  const handleTestimonialSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://tanzimportfolio.web.app/api/testimonials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteTestimonial = async (testimonialId) => {
    ...
    try {
      const res = await fetch(`https://tanzimportfolio.web.app/api/testimonials/${testimonialId}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (res.ok) {
  ...
  const handleDeleteProject = async (projectId) => {
    ...
    try {
      const res = await fetch(`https://tanzimportfolio.web.app/api/projects/${projectId}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (res.ok) {
  ...
  const renderScreenshot = (fileUrl, index) => {
    const fullUrl = `https://tanzimportfolio.web.app${fileUrl}`;
                      ...
                      screenshots.map((screenshot, idx) => (
                        <div key={idx} className="file-preview">
                          {screenshot.toLowerCase().endsWith(".pdf") ? (
                            <iframe
                              src={`https://tanzimportfolio.web.app${screenshot}`}
                              title={`PDF-${idx}`}
                              className="pdf-frame"
                              style={{ cursor: "pointer" }}
                              onClick={() => {
                                setModalItem({
                                  url: `https://tanzimportfolio.web.app${screenshot}`,
                                  isPdf: true,
                                });
                                setZoom(1);
                              }}

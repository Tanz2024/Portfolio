...
const Work = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://tanzimportfolio.web.app/api/projects')
      .then(res => res.json())
      .then(data => {
        setProjects(data);
  ...
  useEffect(() => {
    fetch('https://tanzimportfolio.web.app/authenticate', { credentials: 'include' })
      .then(res => res.json())
      .then(data => {
        if (data.role_id === 1) {
          setIsAdmin(true);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://tanzimportfolio.web.app/api/projects', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
  ...
  const handleDeleteProject = async (id) => {
    ...
    try {
      const res = await fetch(`https://tanzimportfolio.web.app/api/projects/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!res.ok) {
  ...
  const submitEditProject = async (e) => {
    ...
    try {
      const response = await fetch(`https://tanzimportfolio.web.app/api/projects/${editProjectId}`, {
        method: 'PUT',
        credentials: 'include',
                    ...
                    <img
                      ...
                      src={
                        project.image_url.startsWith('/uploads')
                          ? `https://tanzimportfolio.web.app${project.image_url}`
                          : project.image_url
                      }
                      alt={project.title}
                      onClick={() =>
                  ...
                  {project.screenshots && (
                    <div className="project-screenshots">
                      {JSON.parse(project.screenshots).map((fileUrl, index) => {
                        ...
                        const item = {
                          url: `https://tanzimportfolio.web.app${fileUrl}`,
                          isPdf
                        };
                        return (
                            ...
                            {isPdf ? (
                              <iframe
                                ...
                                onClick={() => openModal(JSON.parse(project.screenshots).map(url => ({
                                  url: `https://tanzimportfolio.web.app${url}`,
